...
@click.option('--option', default=1, type=int)
@click.argument('envs', nargs=-1)
def run(load, date, run, option, envs):
    ...
    if load:
        ...
        ### My code start ###
        #####################
        # If option is 2 we need to add action for distinguishing symbol vs non-symbol button press
        ...
        envs = ['./envs/' + env + '.json' for env in envs] # ['./envs/4x4.json'] # './envs/first-experiment.json'
    ...
    environments = [world.World(graph, randomise_observations=True, shiny=(params['shiny'] if np.random.rand() < params['shiny_rate'] else None), option=option) for graph in np.random.choice(envs, params['batch_size'])]
    ...
    for i in range(i_start, params['train_it']):
        ...
        if i % 1000 == 0:
            torch.save(tem.state_dict(), model_path + '/tem_' + str(i) + '_option' + str(option) + '.pt')
            torch.save(tem.hyper, model_path + '/params_' + str(i) + '_option' + str(option) + '.pt')

    # Save the final state of the model after training has finished
    torch.save(tem.state_dict(), model_path + '/tem_' + str(params['train_it'] - 1) + '_option' + str(option) + '.pt')
    torch.save(tem.hyper, model_path + '/params_' + str(params['train_it'] - 1) + '_option' + str(option) + '.pt')
