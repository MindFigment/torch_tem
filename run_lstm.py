...
import model
import plot
from my_utils import make_lstm_directories
import utils
...
def run_lstm(env):
    ...
    run_path, train_path, model_path, save_path, envs_path = make_lstm_directories()
    ...
    lstm = model.LSTM(params['n_x'] + params['n_actions'], 100, params['n_x'], n_a = params['n_actions']).to(device)
    ...
    environments = [world.World(env, 45) for batch in range(params['n_batches'])]
