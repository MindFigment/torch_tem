...
from analyse import performance, location_accuracy
from my_utils import load_model
import world


def test_performance(num_epochs=10):
    ...
    walks = [env.generate_walks(walk_length=params['n_rollout'] * 3, n_walk=1)[0] for env in environments]
    ...
    for epoch in range(1):
        ...
        while all([len(walk) >= params['n_rollout'] for walk in walks]):
            ...
            # Log progress
            print('Finished backprop iter {:d} in {:.2f} seconds.'.format(epoch, time.time() - start_time))
            ...
            all_correct, all_location_frac, all_action_frac = performance(forward, tem, environments)
