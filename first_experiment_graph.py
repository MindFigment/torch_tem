...
def create_first_experiment_graph(env_name, width, height, n_observations, sym2reward, stay_still):
    ...
    # Create dict storing to which reward location each symbol will take us after pressing a button
    ...
    # Temprary dictionaries for creating action transitions for each state
    action1, action2, action3 = dict(), dict(), dict()
    ...
    for rew_loc in range(board_locations, board_locations + n_sym):
        # each reward location can move us to the same places as the symbol it is associated with
        # we don't need to take care of adjacency matrix entry taking us from symbol loction rewloc2symloc[rew_loc] to reward location rew_loc,
        # because now it's analogous to 'stay still' action, unless stay_still == False
        adj[rew_loc, :] = adj[rewloc2symloc[rew_loc], :]
        if not stay_still:
            adj[rew_loc, rew_loc] = 0

        # copy up & down & left & right actions from rewloc2symloc[rew_loc] location
        a_tmp = copy.deepcopy([transition_dict for transition_dict in locations[rewloc2symloc[rew_loc]]['actions'] if transition_dict['name'] not in ['press button', 'stay still']])
        locations[rew_loc]['actions'] += a_tmp
...
def main():
    ...
    env_params['env_name'] = 'first experiment'
    env_params['stay_still'] = False
    # symbols and rewards assiociated with them, ids assigned as last integers of n_observations
    sym2reward = {
        37: 41, # 'banana'
        38: 42, # 'orange'
        39: 43, # 'milk'
    ...
    world_dict = create_first_experiment_graph(**env_params)

    filename = ''.join(['first-experiment', str(world_dict['height']), 'x', str(world_dict['width']), '.json'])
    ...
    # print(world_dict['sym2reward'][37])
    # './envs/first-experiment4x4.json'
    _ = world.World(full_path, randomise_observations=True)
